...
> **What it reviews & who it's for (submission summary):** Second Pass critiques Meta video-ad concepts for laser tattoo removal clinics. It catches the mistakes that make an ad die on the muted scroll — dead hooks, audio-carried messages, discount-led offers, AI-faked proof, guaranteed-results claims that break medical-ad policy — against the direct-response video method. It's an editor, not a rewriter: it points at what's weak, names the standard, and hands it back. **The industry rules live in one swappable profile, so another business can adapt it by adding their own rules** — proven by running a working realtor's real ad through a `real-estate` profile, where the same editor caught Fair Housing risks instead of medical-claim ones, with no changes to the core ([see the receipt](receipts/melissa-old-home-surprises/critique.md)).
